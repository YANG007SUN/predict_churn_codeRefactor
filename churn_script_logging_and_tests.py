...
def test_eda():
	...
	list_of_plots=['Churn_histgram','Customer_Age_histgram','Martital_status','Corr_plot','Total_Trans_Ct']
...
def test_encoder_helper():
	...
	try:
		...
		logging.info('SUCESS: number of rows of X is same as number of rows of raw dataframe')
	except AssertionError as err:
		logging.error('ERROR: number of rows of X is DIFFERENT than number of rows of raw dataframe')
	try:
		assert len(y)==df.shape[0]
		logging.info('SUCESS: number of rows of y is same as number of rows of raw dataframe')
	except AssertionError as err:
		logging.error('ERROR: number of rows of y is DIFFERENT than number of rows of raw dataframe')
...
def test_perform_feature_engineering():
	...
	try:
		...
		logging.info('SUCESS: dimension of X_train,X_test,y_train,y_train are correct.')
	except AssertionError:
		logging.error('ERROR: dimenion of X_train,X_test,y_train,y_train are NOT correct.')
